...
void RegisterList::readCV(char *s) volatile
{
  ...
  for (int i = 0; i < 1; i++)
  {
    ...
    for (int j = 0; j < 500; j++)
    {
      ...
      float abs_ = abs(raw - base);
      Serial.print("abs=");
      Serial.print(abs_);

      current = abs_ * 0.1 + current * (1.0 - 0.1); // compute new exponentially-smoothed current
    ...
    Serial.print("A=");
    Serial.println(current*5/1023/0.185);
